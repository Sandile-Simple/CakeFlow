...
const styles = {
  ...
  notes: { fontSize: 13, color: "#9E7B85", marginTop: 8, fontStyle: "italic" },
  ...
  return (
    <div style={styles.page}>
      ...
      {!loading && orders.length === 0 && (
        <p style={styles.empty}>No orders waiting to be baked right now 🎉</p>
      )}
      ...
      <div style={styles.grid}>
        {orders.map((order) => (
          <div key={order.orderId} style={styles.card}>
            ...
            <button style={styles.button} onClick={() => markAsBaked(order.orderId)}>
              Mark as Baked
            </button>
          </div>
        ))}
